...
async def predict(images: List[UploadFile]) -> Any:
    ...
    """
    image_paths, failed_images = save_images_to_folder(images, data_folder)
    # Save images to the data folder and handle any upload errors
    if failed_images:
...
async def predict_binary(images: List[UploadFile]) -> Any:
    ...
    results = []
    for classification_result in classification_results:
        prediction = "nazi" if classification_result['first_layer_result']['label'] == "nazi-symbol" else "non-nazi"
        confidence = classification_result['first_layer_result']['prob']
        result = dict(prediction=prediction, confidence=confidence)
        results.append(result)
...
async def predict_multiclass(images: List[UploadFile]) -> Any:
    ...
    classification_results = get_second_layer_result(image_paths,
