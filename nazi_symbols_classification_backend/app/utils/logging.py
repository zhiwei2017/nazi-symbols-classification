...
def get_request_msg_args(
    ...
    try:
        ...
        status = "{} {}".format(response_status.value, response_status.phrase)
    except ValueError:
        status = "{} {}".format(response.status_code, "Unknown Error")
    method_path = "{} {} HTTP/{}".format(
        request.method, request.url.path, request.scope["http_version"]
    )
    host, port = "unknown", 0
    if request.client:
        host, port = request.client.host, request.client.port
    args = (host, port, method_path, status, process_time)
    return args
