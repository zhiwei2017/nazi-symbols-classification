...
import click
import logging
from nazi_symbols_classification_backend.app.utils.logging import ColorFormatter, request_msg_format
...
class TestColorFormatter:
    ...
    @mock.patch("nazi_symbols_classification_backend.app.utils.logging.click.style",
    ...
    def test_format_request_msg(self, status, expected_status):
        expected_message = '0.0.0.0:80 - "\x1b[1mGET /dummy/path ' \
        ...
        result = ColorFormatter().format_request_msg(msg=request_msg_format,
                                                     ...
                                                     status=status,
                                                     process_time=0.32)
        assert result == expected_message.format(expected_status)
    ...
    @mock.patch("nazi_symbols_classification_backend.app.utils.logging.logging.Formatter")
    def test_format(self, mocked_formatter, level_no, level_name, color_fmt):
        ...
        log_fmt = '%(asctime)-22.19s \x1b[1m%(name)-21s\x1b[0m ' \
                  '[\x1b[{}m%(levelname)s\x1b[0m]:    ' \
                  '%(message)s    (%(filename)s:%(lineno)d)'
        mocked_formatter.assert_called_with(log_fmt.format(color_fmt))
