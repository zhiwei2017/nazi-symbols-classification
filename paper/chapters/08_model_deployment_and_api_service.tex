...
serialized using \texttt{TorchScript} or the standard PyTorch save format. Classical \ac{ML} models (e.g.,
logistic regression or \ac{SVC} trained on OpenCLIP embeddings) are serialized using \texttt{joblib}. This
preloading strategy reduces runtime latency and avoids repeated I/O overhead.
...
\end{lstlisting}
\caption{Sample \ac{JSON} response for binary classification.}
\label{fig:binary-json-response}
\end{figure}
...
\end{lstlisting}
\caption{Sample \ac{JSON} response for multi-class classification.}
\label{fig:multi-class-json-response}
\end{figure}
...
\end{lstlisting}
\caption{Sample \ac{JSON} response for 2-layer classification.}
\label{fig:two-layer-json-response}
\end{figure}
...
The \ac{API} service is architected for extensibility. Future capabilities—such as object detection for symbol localization, \ac{OCR} for embedded text, or detection of other hate symbols—can be integrated by adding new model pipelines and routes. The decoupled routing logic and memory preloading facilitate straightforward extensibility.
