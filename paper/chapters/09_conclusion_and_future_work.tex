...
including convolutional neural networks (ResNet, EfficientNet), transformer-based architectures (ViT), object
...
the binary task, the best-performing models were OpenCLIP combined with classical classifiers (e.g., SVC and KNN),
ViT, and YOLOv11, all achieving F1-scores above 0.89 and AUC values above 0.94. This indicates that both learned
embeddings and high-resolution feature extractors are effective for distinguishing Nazi symbols from benign content.

In the multi-class setting, YOLOv11 emerged as the top performer, followed closely by ViT. OpenCLIP alone performed
...
challenges in achieving reliable generalization across diverse classes of hate symbols, given their visual variability and contextual ambiguity.
...
\begin{itemize}
    ...
    \item \textbf{Explainability and Robustness:} The integration of explainable AI (XAI) techniques (e.g.,
    Grad-CAM, SHAP) would help interpret classification decisions—especially important in sensitive applications
    ...
    API service for local deployment, future work could involve deploying it in an online setting using cloud
    platforms. This includes optimizing for inference speed, API scalability, and data privacy compliance in
    real-time systems.
...
In summary, this thesis lays a foundation for scalable and practical Nazi symbol classification. Extending it to real-world systems will require continued efforts in model refinement, dataset curation, deployment optimization, and ethical safeguards.
